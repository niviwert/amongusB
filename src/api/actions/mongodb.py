...
from src.api.actions.postgres import add_database, get_database, update_db_name, update_db_status_deleted
...
def delete_mongo_db(deployment_id: str, username: str):
    myclient.drop_database(get_database(deployment_id).get("db_name"))
    update_db_status_deleted(deployment_id)
